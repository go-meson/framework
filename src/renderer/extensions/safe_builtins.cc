...
void SaveImpl(const char* name,
  ...
  context->Global()->SetPrivate(context, MakePrivateKey(name, context->GetIsolate()), value);
}

v8::Local<v8::Object> Load(const char* name, v8::Handle<v8::Context> context) {
  v8::Local<v8::Value> value;
  if (!context->Global()->GetPrivate(context, MakePrivateKey(name, context->GetIsolate())).ToLocal(&value)) {
...
class ExtensionImpl : public v8::Extension {
 public:
  ExtensionImpl()
      : v8::Extension(kClassName, kScript) {}

 private:
  virtual v8::Handle<v8::FunctionTemplate> GetNativeFunctionTemplate(v8::Isolate* isolate, v8::Handle<v8::String> name) override {
