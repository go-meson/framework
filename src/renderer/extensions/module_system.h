...
#include "renderer/extensions/native_handler.h"
...
class ModuleSystem : public ObjectBackedNativeHandler {
 public:
  ...
  // traces.
  v8::Handle<v8::Value> RunString(v8::Handle<v8::String> code, v8::Handle<v8::String> name);
  ...
  // |native_handler|.
  void RegisterNativeHandler(const std::string& name, std::unique_ptr<NativeHandler> native_handler);
 ...
 private:
  ...
  static void LazyFieldGetter(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info);
  // Retrieves the lazily defined field specified by |property| on a native
  // object.
  static void NativeLazyFieldGetter(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info);
  ...
  typedef v8::Handle<v8::Value> (ModuleSystem::*RequireFunction)(const std::string&);
  // Base implementation of a LazyFieldGetter which uses |require_fn| to require
  // modules.
  static void LazyFieldGetterInner(v8::Local<v8::String> property,
  ...
  // Called when an exception is thrown but not caught in JS. Overridable by tests.
