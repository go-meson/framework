...
v8::Local<v8::Private> MakePrivateKey(const char* name, v8::Isolate* isolate) {
  return v8::Private::ForApi(isolate, MakeKey(name, isolate));
}
...
ModuleSystem::ModuleSystem(ScriptContext* context, SourceMap* source_map)
  ...
  global->SetPrivate(isolate->GetCurrentContext(), MakePrivateKey(kModulesField, isolate), v8::Object::New(isolate));
  global->SetPrivate(isolate->GetCurrentContext(), MakePrivateKey(kModuleSystem, isolate), v8::External::New(isolate, this));
}
...
void ModuleSystem::Invalidate() {
  ...
  // and we use this as a signal in lazy handlers that we no longer exist.
  {
    ...
    v8::Handle<v8::Object> global = context()->v8_context()->Global();
    global->DeletePrivate(context()->v8_context(), MakePrivateKey(kModulesField, GetIsolate()));
    global->DeletePrivate(context()->v8_context(), MakePrivateKey(kModuleSystem, GetIsolate()));
  }
...
v8::Local<v8::Value> ModuleSystem::RequireForJsInner(v8::Handle<v8::String> module_name) {
  DLOG(INFO) << __PRETTY_FUNCTION__ << "(" << *v8::String::Utf8Value(module_name) << ")";
  v8::EscapableHandleScope handle_scope(GetIsolate());
  ...
  if (!global->GetPrivate(context()->v8_context(), MakePrivateKey(kModulesField, GetIsolate())).ToLocal(&modules_value)) {
    Warn(GetIsolate(), "Extension view no longer exists(0)");
    return v8::Undefined(GetIsolate());
  }
  if (modules_value.IsEmpty() || modules_value->IsUndefined()) {
    Warn(GetIsolate(), "Extension view no longer exists(1)");
    return v8::Undefined(GetIsolate());
  }
  ...
  v8::Handle<v8::String> wrapped_source(WrapSource(v8::Handle<v8::String>::Cast(source)));
...
void ModuleSystem::RegisterNativeHandler(const std::string& name, std::unique_ptr<NativeHandler> native_handler) {
  native_handler_map_[name] = linked_ptr<NativeHandler>(native_handler.release());
}
...
void ModuleSystem::LazyFieldGetterInner(
  ...
  if (!global->GetPrivate(context, MakePrivateKey(kModuleSystem, info.GetIsolate())).ToLocal(&module_system_value) ||
      module_system_value.IsEmpty() || !module_system_value->IsExternal()) {
    Warn(info.GetIsolate(),
         "Module system has been deleted, does extension view exist?");
    return;
  }

  ModuleSystem* module_system = static_cast<ModuleSystem*>(v8::Handle<v8::External>::Cast(module_system_value)->Value());

  std::string name = *v8::String::Utf8Value(parameters->Get(v8::String::NewFromUtf8(info.GetIsolate(), kModuleName))->ToString());
  ...
  v8::Handle<v8::String> field = parameters->Get(v8::String::NewFromUtf8(info.GetIsolate(), kModuleField))->ToString();

  if (!module->Has(field)) {
    std::string field_str = *v8::String::Utf8Value(field);
    Fatal(module_system->context_, "Lazy require of " + name + "." + field_str + " did not set the " + field_str + " field");
    return;
  }
...
void ModuleSystem::SetLazyField(v8::Handle<v8::Object> object,
  ...
  SetLazyField(object, field, module_name, module_field, &ModuleSystem::LazyFieldGetter);
}
...
void ModuleSystem::SetLazyField(v8::Handle<v8::Object> object,
  ...
  object->SetAccessor(v8::String::NewFromUtf8(GetIsolate(), field.c_str()), getter, nullptr, parameters);
}
...
void ModuleSystem::SetNativeLazyField(v8::Handle<v8::Object> object,
                                      ...
                                      const std::string& module_name,
                                      const std::string& module_field) {
  SetLazyField(object, field, module_name, module_field, &ModuleSystem::NativeLazyFieldGetter);
}

v8::Handle<v8::Value> ModuleSystem::RunString(v8::Handle<v8::String> code, v8::Handle<v8::String> name) {
  return context_->RunScript(name, code, base::Bind(&ExceptionHandler::HandleUncaughtException,
...
void ModuleSystem::Private(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (!args[0]->IsObject() || args[0]->IsNull()) {
    GetIsolate()->ThrowException(
        v8::Exception::TypeError(v8_helpers::ToV8StringUnsafe(GetIsolate(),
                                                              args[0]->IsUndefined()
  ...
  if (!GetPrivate(obj, "privates", &privates) || !privates->IsObject()) {
    ...
    if (privates.IsEmpty()) {
      GetIsolate()->ThrowException(v8_helpers::ToV8StringUnsafe(GetIsolate(), "Failed to create privates"));
      return;
    }
    v8::Maybe<bool> maybe =
