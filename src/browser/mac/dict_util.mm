...
std::unique_ptr<base::ListValue> NSArrayToListValue(NSArray* arr) {
  ...
  for (id value in arr) {
    if ([value isKindOfClass:[NSString class]]) {
      ...
      if (strcmp(objc_type, @encode(BOOL)) == 0 || strcmp(objc_type, @encode(char)) == 0)
        result->AppendBoolean([value boolValue]);
      else if (strcmp(objc_type, @encode(double)) == 0 || strcmp(objc_type, @encode(float)) == 0)
      ...
      std::unique_ptr<base::DictionaryValue> sub_dict = NSDictionaryToDictionaryValue(value);
      if (sub_dict)
...
std::unique_ptr<base::DictionaryValue> NSDictionaryToDictionaryValue(
  ...
  for (id key in dict) {
    std::string str_key = base::SysNSStringToUTF8([key isKindOfClass:[NSString class]] ? key : [key description]);

    id value = [dict objectForKey:key];
    if ([value isKindOfClass:[NSString class]]) {
      result->SetStringWithoutPathExpansion(str_key, base::SysNSStringToUTF8(value));
    } else if ([value isKindOfClass:[NSNumber class]]) {
      const char* objc_type = [value objCType];
      if (strcmp(objc_type, @encode(BOOL)) == 0 || strcmp(objc_type, @encode(char)) == 0)
      ...
      result->SetStringWithoutPathExpansion(str_key, base::SysNSStringToUTF8([value description]));
    }
  }
