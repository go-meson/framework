...
@implementation MesonMenuController
...
- (void)populateWithModel:(meson::MesonMenuModel*)model {
  ...
  for (int index = 0; index < count; index++) {
    if (model->GetTypeAt(index) == meson::MesonMenuModel::TYPE_SEPARATOR)
...
- (NSMenu*)menuFromModel:(meson::MesonMenuModel*)model {
...
- (void)addItemToMenu:(NSMenu*)menu atIndex:(NSInteger)index fromModel:(meson::MesonMenuModel*)model {
  ...
  meson::MesonMenuModel::ItemType type = model->GetTypeAt(index);
  if (type == meson::MesonMenuModel::TYPE_SUBMENU) {
    // Recursively build a submenu from the sub-model at this index.
    [item setTarget:nil];
    [item setAction:nil];
    meson::MesonMenuModel* submenuModel = static_cast<meson::MesonMenuModel*>(model->GetSubmenuModelAt(index));
    ...
    if (model->GetAcceleratorAtWithParams(index, useDefaultAccelerator_, &accelerator)) {
      const ui::PlatformAcceleratorCocoa* platformAccelerator = static_cast<const ui::PlatformAcceleratorCocoa*>(accelerator.platform_accelerator());
      if (platformAccelerator) {
        [item setKeyEquivalent:platformAccelerator->characters()];
        [item setKeyEquivalentModifierMask:platformAccelerator->modifier_mask()];
      }
    }
...
- (BOOL)validateUserInterfaceItem:(id<NSValidatedUserInterfaceItem>)item {
  ...
  meson::MesonMenuModel* model = static_cast<meson::MesonMenuModel*>([[(id)item representedObject] pointerValue]);
  DCHECK(model);
  if (model) {
    ...
    if (model->IsItemDynamicAt(modelIndex)) {
      ...
      NSString* label = l10n_util::FixUpWindowsStyleLabel(model->GetLabelAt(modelIndex));
...
- (void)itemSelected:(id)sender {
  ...
  meson::MesonMenuModel* model = static_cast<meson::MesonMenuModel*>([[sender representedObject] pointerValue]);
  DCHECK(model);
  if (model) {
    NSEvent* event = [NSApp currentEvent];
    model->ActivatedAt(modelIndex, ui::EventFlagsFromModifiers([event modifierFlags]));
  }
}
