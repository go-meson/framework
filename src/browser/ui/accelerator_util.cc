...
#include "common/keyboard_util.h"
...
bool StringToAccelerator(const std::string& shortcut, ui::Accelerator* accelerator) {
  if (!base::IsStringASCII(shortcut)) {
    LOG(ERROR) << "The accelerator string can only contain ASCII characters";
    return false;
  }

  std::vector<std::string> tokens = base::SplitString(shortcut, "+", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
  ...
  for (const auto& token : tokens) {
    bool shifted = false;
    ui::KeyboardCode code = meson::KeyboardCodeFromStr(token, &shifted);
    if (shifted)
...
void GenerateAcceleratorTable(AcceleratorTable* table, meson::MesonMenuModel* model) {
  ...
  for (int i = 0; i < count; ++i) {
    meson::MesonMenuModel::ItemType type = model->GetTypeAt(i);
