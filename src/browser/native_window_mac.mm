...
@interface MesonNSWindowDelegate : NSObject<NSWindowDelegate> {
 @private
  meson::NativeWindowMac* shell_;
  bool is_zooming_;
}
- (id)initWithShell:(meson::NativeWindowMac*)shell;
@end

@implementation MesonNSWindowDelegate
...
@interface MesonNSWindow : EventDispatchingWindow {
 @private
  meson::NativeWindowMac* shell_;
...
- (void)setShell:(meson::NativeWindowMac*)shell;
- (void)setEnableLargerThanScreen:(bool)enable;
- (void)enableWindowButtonsOffset;
@end

@implementation MesonNSWindow
...
- (id)accessibilityAttributeValue:(NSString*)attribute {
  ...
  NSPredicate* predicate = [NSPredicate predicateWithFormat:@"(self isKindOfClass: %@) OR (self.className == %@)", [NSButtonCell class], @"RenderWidgetHostViewCocoa"];
...
- (void)enableWindowButtonsOffset {
  ...
  windowButtonsInterButtonSpacing_ = NSMinX([miniaturizeButton frame]) - NSMaxX([closeButton frame]);

  auto center = [NSNotificationCenter defaultCenter];

  [center addObserver:self selector:@selector(adjustCloseButton:) name:NSViewFrameDidChangeNotification object:closeButton];
  [center addObserver:self selector:@selector(adjustMiniaturizeButton:) name:NSViewFrameDidChangeNotification object:miniaturizeButton];
  [center addObserver:self selector:@selector(adjustZoomButton:) name:NSViewFrameDidChangeNotification object:zoomButton];
}
...
- (void)adjustButton:(NSButton*)button ofKind:(NSWindowButton)kind {
  ...
  buttonFrame.origin = NSMakePoint(offset.x, (NSHeight(frameViewBounds) - NSHeight(buttonFrame) - offset.y));

  switch (kind) {
    case NSWindowZoomButton:
      buttonFrame.origin.x += NSWidth([[self standardWindowButton:NSWindowMiniaturizeButton] frame]);
      buttonFrame.origin.x += windowButtonsInterButtonSpacing_;
    // fallthrough
    case NSWindowMiniaturizeButton:
      buttonFrame.origin.x += NSWidth([[self standardWindowButton:NSWindowCloseButton] frame]);
      buttonFrame.origin.x += windowButtonsInterButtonSpacing_;
    // fallthrough
    default:
      break;
...
@interface MesonProgressBar : NSProgressIndicator
@end

@implementation MesonProgressBar
...
NativeWindowMac::NativeWindowMac(brightray::InspectableWebContents* web_contents, const base::DictionaryValue& options, NativeWindow* parent)
  ...
  options.GetInteger(options::kWidth, &width);
  options.GetInteger(options::kHeight, &height);

  NSRect main_screen_rect = [[[NSScreen screens] objectAtIndex:0] frame];
  NSRect cocoa_bounds = NSMakeRect(round((NSWidth(main_screen_rect) - width) / 2),
                                   round((NSHeight(main_screen_rect) - height) / 2),
                                   width,
                                   height);
  bool resizable = true;
  options.GetBoolean(options::kResizable, &resizable);

  bool minimizable = true;
  options.GetBoolean(options::kMinimizable, &minimizable);

  bool maximizable = true;
  options.GetBoolean(options::kMaximizable, &maximizable);

  bool closable = true;
  options.GetBoolean(options::kClosable, &closable);

  std::string title_bar_style;
  if (options.GetString(options::kTitleBarStyle, &title_bar_style)) {
    if (title_bar_style == "hidden") {
      title_bar_style_ = meson::NativeWindowMac::HIDDEN;
    } else if (title_bar_style == "hiddenInset") {
      title_bar_style_ = meson::NativeWindowMac::HIDDEN_INSET;
    }
  }

  std::string windowType;
  options.GetString(options::kType, &windowType);

  bool useStandardWindow = true;
  // eventually deprecate separate "standardWindow" option in favor of
  // standard / textured window types
  options.GetBoolean(options::kStandardWindow, &useStandardWindow);
  ...
  window_.reset([[MesonNSWindow alloc] initWithContentRect:cocoa_bounds
                                                 styleMask:styleMask
  ...
  window_delegate_.reset([[MesonNSWindowDelegate alloc] initWithShell:this]);
  [window_ setDelegate:window_delegate_];

  DLOG(INFO) << __PRETTY_FUNCTION__ << " window created\n";
  ...
  bool focusable = true;
  if (options.GetBoolean(options::kFocusable, &focusable) && !focusable)
    [window_ setDisableKeyOrMainWindow:YES];
  ...
  options.GetBoolean(options::kAcceptFirstMouse, &acceptsFirstMouse);
  [window_ setAcceptsFirstMouse:acceptsFirstMouse];

  // Disable auto-hiding cursor.
  bool disableAutoHideCursor = false;
  options.GetBoolean(options::kDisableAutoHideCursor, &disableAutoHideCursor);
  ...
  wheel_event_monitor_ = [NSEvent addLocalMonitorForEventsMatchingMask:NSScrollWheelMask
  ...
  RegisterInputEventObserver(web_contents->GetWebContents()->GetRenderViewHost());
}
...
void NativeWindowMac::SetProgressBar(double progress, const NativeWindow::ProgressState state) {
  ...
  if ([[dock_tile.contentView subviews] count] == 0) {
    NSProgressIndicator* progress_indicator = [[MesonProgressBar alloc]
...
void NativeWindowMac::RenderViewHostChanged(content::RenderViewHost* old_host, content::RenderViewHost* new_host) {
...
void NativeWindowMac::RegisterInputEventObserver(content::RenderViewHost* host) {
  if (host)
    host->GetWidget()->AddInputEventObserver(this);
}

void NativeWindowMac::UnregisterInputEventObserver(content::RenderViewHost* host) {
  if (host)
    host->GetWidget()->RemoveInputEventObserver(this);
}

// static
NativeWindow* NativeWindow::Create(brightray::InspectableWebContents* inspectable_web_contents,
